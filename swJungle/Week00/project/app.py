...
def show_memos():
    # 1. 모든 memo 찾기
    results = list(db.TodaysCafeteriaMenu.find({}))
    allComments = []
    for result in results:
      information = {'_id':str(result['_id']), 'comment' : result['comment'], 'star' : result['star']}
      allComments.append(information)
    # 2. memos라는 키 값으로 정보 내려주기
    return jsonify({'result':'success','memos':allComments,'msg':'조회 완료!'})
...
def post_comment():
    ...
    # 2. mongoDB 에 입력
    comment = {'title': title_receive, 'comment': comment_receive, 'star' : star_receive}
		
    db.TodaysCafeteriaMenu.insert_one(comment)
...
def delete_memo():
    ...
    # id와 일치하는 메모 삭제
    db.TodaysCafeteriaMenu.delete_one({'_id':id_delete})
